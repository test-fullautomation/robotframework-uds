...
The \pkg\ provides a versatile "Send UDS Request By Name" keyword that can send 
and receive UDS messages without requiring the command type specification 
(e.g., Read, Write, RoutineControl). This allows users to send requests without 
needing to know the specific UDS command type associated with a service, as the 
keyword automatically detects and handles it.

{\large\textbf{Examples}}

\textbf{Send UDS Request By Name}
\begin{robotcode}
...
\textbf{Input Output Control By Name}
\begin{robotcode}
